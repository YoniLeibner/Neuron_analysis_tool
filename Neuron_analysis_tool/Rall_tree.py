...
def create_tree(start=dict(x=0, y=0, z=0), max_depth=3, Rm=10000.0, Ra=100.0, E_length=0.25, n_samples=10):
    ...
    helping(f, 0, max_depth=max_depth, start=dict(x=0, y=0, z=0), num_to_create=1, diam=10.0, Rm=Rm, Ra=Ra,
...
def helping(f, depth, max_depth=3, start={'x': 0, 'y': 0, 'z': 0}, num_to_create=1, diam=1.0, Rm=10000.0, Ra=100.0,
    ...
    if num_to_create == 1:
        ...
        # start_soma = deepcopy(start)
        # start_soma['x']-=5
        start['z'] += EPS
        # point, num = section(f, start_soma, start, 10, num + 1, prev_num, type=1, n_samples=2)
        point, num = section(f, start, end, diam, num + 1, prev_num, type=1, n_samples=n_samples)

        # soma_num = num
        # basal_end = deepcopy(start)
        # basal_end['z'] -= 2*EPS
        # point, num = section(f, start, basal_end, diam, num + 1, 1, type=3, n_samples=n_samples)
...
create_tree(start={'x': 0, 'y': 0, 'z': 0}, max_depth=depth, Rm=10000.0, Ra=100.0, E_length=0.25, n_samples=10)
